...

{
	...
	{	// just use the given name
		...
		LPCWSTR pipestr = temp.c_str();

	//std::string pipeS = &m_name[0];
	// Create the pipe
	...
	std::string pipeProc[MAX_PATH];;

	if (!helper_executable)
		helper_executable = DEFAULT_HELPER_EXE;

	BOOL bRet = CreateProcess(NULL,L"pipeProc", NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi);
	if (!bRet)
...
//			sprintf(cmdline, "%s %s", path, logger_name);
			...
			bRet = CreateProcess(NULL, L"stringProc", NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi);
		}
		if (!bRet)
		{
